...
# This worksheet computes the minmial polynomial of the cogrowth OGF for Z_2^{*m}*Z_d, for vaious d>2, and parameter m. Leading terms are compared via producing LaTex code for a table.
...
get_input_file := proc(d) local dirloc; description "gets the name of the file to read containing the FD variables and system of equations", "the file to read contains 2 items", "syst: system of eqns, varlist: list of variable names used in the system", "the first variable, varlist[1], gives the cogrowth sequence"; dirloc := "maple_data_files/"; return sprintf("%sZ2_m__Z%d_1__ic-dat.maple", dirloc, d); end proc:
get_output_file := proc(d) local dirloc; description "gets the name of the file to store series and min poly"; dirloc := "expl_sols/"; return sprintf("%sexpl_sol_Z2_m__Z%d_1__ic.mpl", dirloc, d); end proc:
...
getLTMinPoly := proc(d) read get_output_file(d); return lcoeff(ff, z); end proc:


# compute for some prime d
for d in [3,5,7,11,13,17] do
